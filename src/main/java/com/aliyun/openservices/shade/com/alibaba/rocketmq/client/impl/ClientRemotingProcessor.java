...
public class ClientRemotingProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx,
        ...
        switch (request.getCode()) {
            ...
            case RequestCode.PULL_METRIC_DATA:
                return this.pullMetricsData(ctx, request);
            default:
                break;
        }
        return null;
    ...
    public RemotingCommand checkTransactionState(ChannelHandlerContext ctx,
        ...
        final ByteBuffer byteBuffer = request.getBody();
    ...
    public RemotingCommand resetOffset(ChannelHandlerContext ctx,
        ...
        if (request.getBody() != null) {
            ResetOffsetBody body = JsonUtils.decode(request.getBody(), ResetOffsetBody.class);
    ...
    private RemotingCommand getConsumerRunningInfo(ChannelHandlerContext ctx,
        ...
        if (null != consumerRunningInfo) {
            ...
            if (requestHeader.isMetricsEnable()) {
                try {
                    String metrics = MetricsUtils.metrics2String();
                    consumerRunningInfo.setMetrics(metrics);
                } catch (Exception e) {
                    log.error("fetch metrics data failed.", e);
                }
            }
    ...
    private RemotingCommand consumeMessageDirectly(ChannelHandlerContext ctx,
        ...
        final MessageExt msg = MessageDecoder.decode(request.getBody());
    ...
    private RemotingCommand pullMetricsData(ChannelHandlerContext ctx,
                                            RemotingCommand request) throws RemotingCommandException {
        final RemotingCommand response = RemotingCommand.createResponseCommand(PullMetricsDataResponseHeader.class);
        final PullMetricsDataResponseHeader responseHeader = (PullMetricsDataResponseHeader) response.readCustomHeader();
        final PullMetricsDataRequestHeader requestHeader =
                (PullMetricsDataRequestHeader) request.decodeCommandCustomHeader(PullMetricsDataRequestHeader.class);
        String token = requestHeader.getToken();
        responseHeader.setCompressed(false);
        response.setCode(ResponseCode.SYSTEM_ERROR);
        try {
            if (!MetricsUtils.addTokens(token)) {
                response.setRemark("Token is not available now.");
            }
            String metrics = MetricsUtils.metrics2String();
            MetricsData body = new MetricsData();
            body.setMetrics(metrics);
            boolean compressed = false;
            if (metrics.length() > MetricsUtils.COMPRESSION_MIN_SIZE) {
                compressed = true;
            }
            responseHeader.setCompressed(compressed);
            response.setBody(body.encode(compressed));
            response.setCode(ResponseCode.SUCCESS);
        } catch (Exception e) {
            log.error("fetch metrics data failed, msg: {}", e.getMessage());
            response.setRemark("Fetch metrics data failed.");
        }
        return response;
    }
}
