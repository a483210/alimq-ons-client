...
public class RemotingCommand {
    ...
    private transient final AtomicBoolean copied = new AtomicBoolean(false);
    ...
    private static RemotingCommand headerDecode(byte[] headerData, SerializeType type) {
        switch (type) {
            case JSON:
                RemotingCommand resultJson = JsonUtils.decode(headerData, RemotingCommand.class);
    ...
    public ByteBuffer encodeHeader() {
        ...
        if (null != payload) {
            bodySize = payload.readableBytes();
        }
        return encodeHeader(bodySize);
    }
    ...
    /**
     * We cannot simply return the backing up array as the returned array does not respect position and limit
     *
     * @return array of the readable data.
     */
    @JsonIgnore
    public byte[] getByteArrayBody() {
        ...
        if (!RemotingCopyElisionRequestCode.COPY_ELISION_LIST.contains(code)) {
            return body.array();
        }
        ...
        ByteBuffer buffer = ByteBuffer.allocate(body.remaining());
    ...
    public ByteBuf encodeFrame() {
        ...
        ByteBuffer buffer = encodeHeader();
        frame.addComponent(true, Unpooled.wrappedBuffer(buffer));
        if (null != body) {
            frame.addComponent(true, Unpooled.wrappedBuffer(body));
        } else if (null != payload) {
            frame.addComponent(true, getPayload());
        }
        return frame;
    }

    /**
     * readIndex and writeIndex of payload is always protected. Note, the underlying reference counter is shared among
     * the shallow copies.
     *
     * @return Shallow shared copy of the payload data.
     */
    @JsonIgnore
     ...
     *
     * @return accessibility of the payload.
     */
    public boolean payloadAccessible() {
        return null != payload && payload.refCnt() != 0;
    }
