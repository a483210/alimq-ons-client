...
public class MixAll {
    ...
    public static void unsafeWrite(InputStream inputStream, File file) throws IOException {
        File parent = file.getParentFile();
        if (!parent.exists()) {
            if (!parent.mkdirs()) {
                log.warn("Failed to create parent directories: {}", parent.getCanonicalPath());
        ...
        if (!file.exists()) {
            if (!file.createNewFile()) {
                log.warn("Failed to create new file: {}", file.getCanonicalPath());
    ...
    public static void safeWrite(InputStream inputStream, final String fileName) throws IOException {
        File tmpFile = new File(fileName + ".tmp");
        unsafeWrite(inputStream, tmpFile);
    ...
    public static Properties byteBuffer2Properties(final ByteBuffer buffer) {
    ...
    public static String execCommand(String[] command) {
        ...
        try {
            ...
            log.error("execCommand error" + Arrays.toString(command));
            e.printStackTrace();
        }
        return null;
    }
    ...
    public static String execCurl(String url) {
        ...
        String[] commands = {"curl", "-X", "GET", url};
