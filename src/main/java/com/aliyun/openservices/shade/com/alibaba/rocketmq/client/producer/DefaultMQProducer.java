...
public class DefaultMQProducer extends ClientConfig implements MQProducer {
    ...
    public SendResult send(Message msg) throws MQClientException, RemotingException, MQBrokerException,
        ...
        if (null == msg) {
            throw new MQClientException(ResponseCode.MESSAGE_ILLEGAL, "Message is null");
        }
    ...
    /**
     * Same to {@link #send(Message)} with total send times specified in addition.
     *
     * @param msg message to send.
     * @param sendTimesTotal total send times
     * @param initLastBrokerName initial last broker name, set to avoid send msg to this broker in the first time
     * @return {@link SendResult} instance to inform senders details of the deliverable, say Message ID of the message,
     * {@link SendStatus} indicating broker storage/replication status, message queue sent to, etc.
     * @throws MQClientException if there is any client error.
     * @throws RemotingException if there is any network-tier error.
     ...
     */
    @Override
    public SendResult send(Message msg, Integer sendTimesTotal, String initLastBrokerName) throws MQClientException, RemotingException, MQBrokerException,
        InterruptedException {
        if (null == msg) {
            throw new MQClientException(ResponseCode.MESSAGE_ILLEGAL, "Message is null");
        }
        msg.setTopic(withNamespace(msg.getTopic()));
        return this.defaultMQProducerImpl.send(msg, sendTimesTotal, initLastBrokerName);
    }
    ...
    public long searchOffset(MessageQueue mq, long timestamp, BoundaryType boundaryType) throws MQClientException {
        return this.defaultMQProducerImpl.searchOffset(queueWithNamespace(mq), timestamp, boundaryType);
    }
     ...
     * If the Executor is not set,  will be used.
     *
    ...
    public void detectByOneRound() {
        this.defaultMQProducerImpl.detectByOneRound();
    }
    ...
    public void setIfStartDetector(final boolean startDetectorEnable) {
        this.defaultMQProducerImpl.setIfStartDetector(startDetectorEnable);
    }
