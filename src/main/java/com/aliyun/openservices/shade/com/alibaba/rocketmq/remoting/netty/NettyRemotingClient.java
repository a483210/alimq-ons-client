...
import com.aliyun.openservices.shade.com.alibaba.rocketmq.remoting.*;
...
import com.aliyun.openservices.shade.com.alibaba.rocketmq.remoting.statistics.Histogram;
...
public class NettyRemotingClient extends NettyRemotingAbstract implements RemotingClient {
    ...
    private final Map<String /*cidr*/, ProxyConfig/*proxy*/> proxyMap = new HashMap<String, ProxyConfig>();
    private final ConcurrentHashMap<String /*cidr*/, Bootstrap> bootstrapMap = new ConcurrentHashMap<String, Bootstrap>();
    ...
    private final ConcurrentMap<Integer, Histogram> rpcLatencyHistogram;
    ...
    public NettyRemotingClient(final NettyClientConfig nettyClientConfig,
        ...
        Map<String, ProxyConfig> sockProxyMap;
        try {
            sockProxyMap = JsonUtils.mapper.readValue(nettyClientConfig.getSockProxyJson(), new TypeReference<Map<String, ProxyConfig>>() {
            });
        ...
        rpcLatencyHistogram = new ConcurrentHashMap<Integer, Histogram>();
        String[] latencyLabels = {
                "0000ms~0010ms",
                "0010ms~0050ms",
                "0050ms~0100ms",
                "0100ms~0300ms",
        ...
        String title = String.format("Latency Distribution of Code %s", RemotingCopyElisionRequestCode.REGISTER_BROKER);
        rpcLatencyHistogram.put(RemotingCopyElisionRequestCode.REGISTER_BROKER, new Histogram(title, latencyLabels));
    }

    private int latencyToGrade(long latency) {
        int grade;
        if (latency < 10) {
            grade = 0;
        } else if (latency < 50) {
    ...
    protected void collectLatencyStatistics(int code, long latency) {
        if (RemotingCopyElisionRequestCode.REGISTER_BROKER == code) {
            rpcLatencyHistogram.get(code).add(latencyToGrade(latency));
        }
    }
    ...
    public void start() {
        ...
        WriteBufferWaterMark writeBufferWaterMark =
                new WriteBufferWaterMark(nettyClientConfig.getWriteBufferLowWaterMark(),
        ...
        this.timer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    log.error("scanExpiredMetricTimer exception", e);
                }
            }
        }, 1000 * 30, 1000 * 60);
        ...
        this.timer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                try {
                    for (Map.Entry<Integer, Histogram> entry : rpcLatencyHistogram.entrySet()) {
                        String report = entry.getValue().report(true);
                        log.info("RPC Latency Report: {}", report);
                    }
                } catch (Exception e) {
                    log.error("rpcLatencyReport exception", e);
                }
            }
        }, 30 * 1000, 30 * 1000);
    ...
    private Bootstrap fetchBootstrap(String addr) {
        ...
        log.info("addr: {}, cidr: {}, proxy: {}", addr, cidr, proxyConfig != null ? proxyConfig.getAddr() : "");
                ...
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    public void initChannel(SocketChannel ch) {
                        ...
                        if (proxy != null) {
                            ...
                            pipeline.addFirst(new Socks5ProxyHandler(new InetSocketAddress(arr[0], Integer.parseInt(arr[1])), proxy.getUsername(), proxy.getPassword()));
                        }
        ...
        try {
            if (this.lockChannelTables.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    if (null == prevCW) {
                        log.debug("eventCloseChannel: the channel was removed from the channel table");
                        removeItemFromTable = false;
                    }
    ...
    public void updateNameServerAddressList(List<String> addressList) {
        ...
        if (!addressList.isEmpty()) {
            ...
            if (update) {
                ...
                this.namesrvAddrList.set(addressList);

                // should close the channel if chosen addr does not exist.
                if (this.namesrvAddrChosen.get() != null && !addressList.contains(this.namesrvAddrChosen.get())) {
                    ChannelWrapper channelWrapper = this.channelTables.get(this.namesrvAddrChosen.get());
    ...
    public RemotingCommand invokeSync(String addr, final RemotingCommand request, long timeoutMillis)
        ...
        if (channel != null && channel.isActive()) {
            try {
                doBeforeRequests(channel, request);
                RemotingCommand response = this.invokeSyncImpl(channel, request, timeoutMillis);
                doAfterResponses(request, channel, response);
                this.updateChannelLastResponseTime(addr);
                return response;
            } catch (RemotingSendRequestException e) {
                log.warn("invokeSync: send request exception, so close the channel[{}]", addr);
                doAfterRpcFailure(request, channel, false);
                ...
                doAfterRpcFailure(request, channel, true);
    ...
    public void doAfterRpcFailure(RemotingCommand request, Channel channel, Boolean remoteTimeout) {
        if (rpcHookList.isEmpty()) {
            return;
        }

        for (RPCHook rpcHook : rpcHookList) {
            rpcHook.doAfterRpcFailure(RemotingHelper.parseChannelRemoteAddr(channel), request, remoteTimeout);
        }
    }

    public void doAfterResponses(RemotingCommand request, Channel channel, RemotingCommand response) {
        if (rpcHookList.isEmpty()) {
            return;
        }
    ...
    private void doBeforeRequests(Channel channel, RemotingCommand request) {
        if (rpcHookList.isEmpty()) {
            return;
        }

        for (RPCHook rpcHook : rpcHookList) {
            rpcHook.doBeforeRequest(RemotingHelper.parseChannelRemoteAddr(channel), request);
        }
    }
    ...
    private void updateChannelLastResponseTime(final String addr) {
        String address = addr;
        if (address == null) {
            address = this.namesrvAddrChosen.get();
        }
        if (address == null) {
    ...
    private void scanExpiredMetricTimer() {
        for (RPCHook rpcHook : rpcHookList) {
            if (rpcHook instanceof RpcMetricHook) {
                RpcMetricHook rpcMetricHook = (RpcMetricHook) rpcHook;
                int expiredTimerNum = rpcMetricHook.cleanExpiredTimer();
                if (expiredTimerNum > 0) {
                    log.warn("{} expired metrics timer has been deleted.", expiredTimerNum);
                }
            }
        }
    }
