...
public class MQAdminImpl {
    ...
    private static final long THREE_DAY_MILLISECONDS = 3L * 24L * 60L * 60L * 1000L;

    private long getBeginTimeStamp() {
        return System.currentTimeMillis() - THREE_DAY_MILLISECONDS;
    }
    ...
    public List<MessageQueue> parsePublishMessageQueues(List<MessageQueue> messageQueueList) {
        ...
        for (MessageQueue queue : messageQueueList) {
            ...
            resultQueues.add(new MessageQueue(userTopic, queue.getBrokerName(), queue.getQueueId(), queue.getQueueGroupId(), queue.isMainQueue()));
        }
        ...
        if (brokerAddr != null) {
            try {
                return this.mQClientFactory.getMQClientAPIImpl().searchOffset(brokerAddr, mq.getTopic(),
                    mq.getQueueId(), timestamp, timeoutMillis, boundaryType);
            } catch (Exception e) {
    ...
    /**
     * Return the max committed or inflight offset of the specific queue.
     ...
     *
     * @param mq the specific queue
     * @param committed true if the max committed offset is needed, or false if the max inflight offset is preferred.
     * @return the max offset
     * @throws MQClientException if client exception occurred
     */
    public long maxOffset(MessageQueue mq, boolean committed) throws MQClientException {
        ...
        if (brokerAddr != null) {
            try {
                return this.mQClientFactory.getMQClientAPIImpl().getMaxOffset(brokerAddr, mq.getTopic(), mq.getQueueId(), committed, timeoutMillis);
            } catch (Exception e) {
                throw new MQClientException("Invoke Broker[" + brokerAddr + "] exception", e);
            }
        }
    ...
    public MessageExt queryMessageByUniqKey(String topic,
        ...
        return queryMessageByUniqKey(topic, uniqKey, getBeginTimeStamp(), Long.MAX_VALUE);
    }

    public MessageExt queryMessageByUniqKey(String clusterName, String topic,
                                            String uniqKey) throws InterruptedException, MQClientException {
        return queryMessageByUniqKey(clusterName, topic, uniqKey, getBeginTimeStamp(), Long.MAX_VALUE);
    }
    ...
    public MessageExt queryMessageByUniqKey(String clusterName, String topic,
        String uniqKey, long begin, long end) throws InterruptedException, MQClientException {
        QueryResult qr = queryAllMessageByUniqKey(clusterName, topic, uniqKey, begin, end);
    ...
    public QueryResult queryAllMessageByUniqKey(String clusterName, String topic, String uniqKey)
        throws MQClientException, InterruptedException {
        return this.queryAllMessageByUniqKey(clusterName, topic, uniqKey, getBeginTimeStamp(), Long.MAX_VALUE);
    }

    public QueryResult queryAllMessageByUniqKey(String clusterName, String topic,
        String uniqKey, long begin, long end) throws MQClientException, InterruptedException {
        return this.queryMessage(clusterName, topic, uniqKey, 32, begin, end, true, true);
    }
    ...
    protected QueryResult queryMessage(String topic, String key, int maxNum, long begin, long end,
        ...
        InterruptedException {
        return queryMessage(null, topic, key, maxNum, begin, end, isUniqKey, false);
    }

    protected QueryResult queryMessage(String clusterName, String topic, String key, int maxNum, long begin, long end,
        boolean isUniqKey, boolean showAll) throws MQClientException,
        InterruptedException {
                ...
                for (String addr : brokerAddrs) {
                    try {
                        ...
                        this.mQClientFactory.getMQClientAPIImpl().queryMessage(addr, requestHeader, timeoutMillis * 3,
                            new BaseInvokeCallback(this.mQClientFactory.getMQClientAPIImpl()) {
                                @Override
                                            ...
                                            switch (response.getCode()) {
                                                case ResponseCode.SUCCESS: {
                                                    ...
                                                    List<MessageExt> wrappers =
                                                        MessageDecoder.decodes(response.getBody(), true);
