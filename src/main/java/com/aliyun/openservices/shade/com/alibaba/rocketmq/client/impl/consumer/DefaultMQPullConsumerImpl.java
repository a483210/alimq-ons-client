...
public class DefaultMQPullConsumerImpl implements MQConsumerInner, MQPopConsumer {
    ...
    public long fetchConsumeOffset(MessageQueue mq, boolean fromStore) throws MQClientException {
        this.makeSureStateOK();
        try {
            final Optional<Long> optionalOffset = this.offsetStore.readOffset(mq, fromStore ?
            ...
            throw new MQClientException("Failed to read offset", t);
        }
    }
    ...
    public PullResult pull(MessageQueue mq, PullMessageSelector messageSelector)
        ...
        return this.pullSyncImpl(mq, messageSelector.getExpression(), messageSelector.getType(), messageSelector.getOffset(),
            messageSelector.getMaxNums(), messageSelector.isBlockIfNotFound(), timeout, messageSelector.getPropertiesStr());
    }
    ...
    public void pull(MessageQueue mq, PullMessageSelector messageSelector, PullCallback callback)
        ...
        this.pullAsyncImpl(mq, messageSelector.getExpression(), messageSelector.getType(), messageSelector.getOffset(),
            messageSelector.getMaxNums(), callback,
            messageSelector.isBlockIfNotFound(), timeout, messageSelector.getPropertiesStr());
    }
    ...
    public PullResult pull(MessageQueue mq, String subExpression, long offset, int maxNums, long timeout)
        throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        return this.pullSyncImpl(mq, subExpression, ExpressionType.TAG, offset, maxNums, false, timeout, null);
    }

    private PullResult pullSyncImpl(MessageQueue mq, String subExpression, final String expressionType, long offset, int maxNums, boolean block, long timeout, String subProperties)
        throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        this.makeSureStateOK();
        ...
        try {
            subscriptionData = FilterAPI.build(mq.getTopic(), subExpression, expressionType);
        } catch (Exception e) {
    ...
    public boolean doRebalance() {
        ...
        if (this.rebalanceImpl != null) {
            balanced = this.rebalanceImpl.doRebalance(false);
        }
        return balanced;
    }
    ...
    public void pull(MessageQueue mq, String subExpression, long offset, int maxNums, PullCallback pullCallback,
        long timeout)
        throws MQClientException, RemotingException, InterruptedException {
        this.pullAsyncImpl(mq, subExpression, ExpressionType.TAG, offset, maxNums, pullCallback, false, timeout, null);
    }
    ...
    private void pullAsyncImpl(
        ...
        final boolean block,
        final long timeout,
        final String subProperties) throws MQClientException, RemotingException, InterruptedException {
        this.makeSureStateOK();
    ...
    public PullResult pullBlockIfNotFound(MessageQueue mq, String subExpression, long offset, int maxNums)
        throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        return this.pullSyncImpl(mq, subExpression, ExpressionType.TAG, offset, maxNums, true, this.getDefaultMQPullConsumer().getConsumerPullTimeoutMillis(), null);
    }

    public void pullBlockIfNotFound(MessageQueue mq, String subExpression, long offset, int maxNums,
        PullCallback pullCallback)
        throws MQClientException, RemotingException, InterruptedException {
        this.pullAsyncImpl(mq, subExpression, ExpressionType.TAG, offset, maxNums, pullCallback, true,
            this.getDefaultMQPullConsumer().getConsumerPullTimeoutMillis(), null);
    }

    public PullResult pullBlockIfNotFound(MessageQueue mq, PullMessageSelector messageSelector)
            throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        return this.pullSyncImpl(mq, messageSelector.getExpression(), messageSelector.getType(), messageSelector.getOffset(), messageSelector.getMaxNums(), true, this.getDefaultMQPullConsumer().getConsumerPullTimeoutMillis(), null);
    }


    public void pullBlockIfNotFound(MessageQueue mq, PullMessageSelector messageSelector, PullCallback pullCallback)
            throws MQClientException, RemotingException, InterruptedException {
        this.pullAsyncImpl(mq, messageSelector.getExpression(), messageSelector.getType(), messageSelector.getOffset(), messageSelector.getMaxNums(), pullCallback, true,
                this.getDefaultMQPullConsumer().getConsumerPullTimeoutMillis(), null);
    }
    ...
    public void popAsync(MessageQueue mq, long invisibleTime, int maxNums, String consumerGroup, long timeout, PopCallback popCallback, boolean poll, int initMode) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        popAsync(mq, invisibleTime, maxNums, consumerGroup, timeout, popCallback, poll, initMode, null, null);
    }

    @Override public void popAsync(MessageQueue mq, List<Integer> queueIdList, long invisibleTime, int maxNums,
        String consumerGroup, long timeout, PopCallback popCallback, boolean poll,
        int initMode) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        popAsync(mq, queueIdList, invisibleTime, maxNums, consumerGroup, timeout, popCallback, poll, initMode, null, null);
    }
    ...
    public void popAsync(MessageQueue mq, long invisibleTime, int maxNums, String consumerGroup, long timeout, PopCallback popCallback, boolean poll, int initMode, String expressionType, String expression) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        this.subscriptionAutomatically(mq.getTopic());
        popAsync(mq, null, invisibleTime, maxNums, consumerGroup, timeout, popCallback, poll, initMode, expressionType, expression);
    }

    @Override
    public void popAsync(MessageQueue mq, List<Integer> queueIdList, long invisibleTime, int maxNums, String consumerGroup, long timeout, PopCallback popCallback, boolean poll, int initMode, String expressionType, String expression) throws MQClientException, RemotingException, MQBrokerException, InterruptedException {
        this.subscriptionAutomatically(mq.getTopic());
        FindBrokerResult findBrokerResult = this.mQClientFactory.findBrokerAddressInSubscribe(mq.getBrokerName(), MixAll.MASTER_ID, true);
        if (null == findBrokerResult) {
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(mq.getTopic());
        ...
        if (findBrokerResult != null) {
            ...
            if (queueIdList != null && !queueIdList.isEmpty()) {
                StringBuilder queueIdStringBuilder = new StringBuilder();
                for (int i = 0; i < queueIdList.size(); i++) {
                    int queueId = queueIdList.get(i);
                    queueIdStringBuilder.append(queueId);
                    if (i != queueIdList.size() - 1) {
                        queueIdStringBuilder.append(',');
                    }
                }
                requestHeader.setQueueIdListString(queueIdStringBuilder.toString());
            }
            requestHeader.setExpType(expressionType);
            requestHeader.setExp(expression);
            //give 1000 ms for server response
