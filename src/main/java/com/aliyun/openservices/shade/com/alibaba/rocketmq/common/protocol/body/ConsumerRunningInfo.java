...
public class ConsumerRunningInfo extends RemotingSerializable {
    ...
    public String formatString() {
        ...
        {
            sb.append("\n\n#Consumer Offset#\n");
            boolean consumeOrderly = Boolean.parseBoolean(properties.getProperty(ConsumerRunningInfo.PROP_CONSUME_ORDERLY));
            if (!consumeOrderly) {
                ...
                sb.append(String.format("%-32s  %-32s  %-4s  %-4s  %-20s  %-20s%n",
                        "#Topic",
                        "#Broker Name",
                        "#QID",
                        "#GID",
                ...
                Map<String, Map<Integer, List<Pair<MessageQueue, ProcessQueueInfo>>>> topicQueueGroupTable =
                        new HashMap<String, Map<Integer, List<Pair<MessageQueue, ProcessQueueInfo>>>>();

                for (MessageQueue mq : mqTable.keySet()) {
                    if (!topicQueueGroupTable.containsKey(mq.getTopic())) {
                        topicQueueGroupTable.put(mq.getTopic(), new HashMap<Integer, List<Pair<MessageQueue, ProcessQueueInfo>>>());
                    }
                    Map<Integer, List<Pair<MessageQueue, ProcessQueueInfo>>> queueGroupTable =
                            topicQueueGroupTable.get(mq.getTopic());

                    if (!queueGroupTable.containsKey(mq.getQueueGroupId())) {
                        queueGroupTable.put(mq.getQueueGroupId(), new ArrayList<Pair<MessageQueue, ProcessQueueInfo>>());
                    }
                    List<Pair<MessageQueue, ProcessQueueInfo>> pqInfoList = queueGroupTable.get(mq.getQueueGroupId());
                    pqInfoList.add(new Pair<MessageQueue, ProcessQueueInfo>(mq, mqTable.get(mq)));
                    Collections.sort(pqInfoList, new Comparator<Pair<MessageQueue, ProcessQueueInfo>>() {
                        @Override
                        public int compare(Pair<MessageQueue, ProcessQueueInfo> o1,
                                           Pair<MessageQueue, ProcessQueueInfo> o2) {
                            return o1.getObject1().compareTo(o2.getObject1());
                        }
                    });
                }

                for (Map.Entry<String, Map<Integer, List<Pair<MessageQueue, ProcessQueueInfo>>>> entry :
                        topicQueueGroupTable.entrySet()) {
                    String topic = entry.getKey();
                    for (Map.Entry<Integer, List<Pair<MessageQueue, ProcessQueueInfo>>> queueGroupEntry :
                            entry.getValue().entrySet()) {
                        int queueGroupId = queueGroupEntry.getKey();
                        for (Pair<MessageQueue, ProcessQueueInfo> queuePair : queueGroupEntry.getValue()) {
                            MessageQueue mq = queuePair.getObject1();
                            ProcessQueueInfo pqInfo = queuePair.getObject2();
                            String item = String.format("%-32s  %-32s  %-4d  %-4s  %-20d  %-20s%n",
                                    topic,
                                    mq.getBrokerName(),
                                    mq.getQueueId(),
                                    queueGroupId == MessageQueue.NO_QUEUE_GROUP ? "N/A" : String.valueOf(queueGroupId),
                                    pqInfo.getCommitOffset(),
                                    queueGroupId == MessageQueue.NO_QUEUE_GROUP ? "N/A" : String.valueOf(pqInfo.getMergeOffset()));
